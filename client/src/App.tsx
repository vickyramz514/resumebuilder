...
function App() {
  ...
  const { selectedSection, setSelectedSection, setSections, reorderSections, setTemplate, updateResume, resumes, activeId, setActive, addResume, duplicateResume, deleteResume } = useResumeStore();
  ...
  const exportPdf = async () => {
    ...
    const response = await fetch('/api/pdf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ resume, html: `<html><head><style>${css}</style></head><body>${node.outerHTML}</body></html>` }) });
