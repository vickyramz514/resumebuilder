...
import { Alert, AppBar, Box, Button, Chip, Divider, Drawer, FormControl, IconButton, InputLabel, Menu, MenuItem, Select, Slider, Stack, Tab, Tabs, Toolbar, Tooltip, Typography, useMediaQuery } from '@mui/material';
import { Download, FilePlus2, FileText, Menu as MenuIcon, MoreVertical, Palette, PanelLeftClose, PanelLeftOpen, Plus, Sparkles } from 'lucide-react';
...
function ResumeBuilder() {
  ...
  const editorPanel = <Box className="editor-panel"><Box className="editor-heading"><Typography variant="h6">{selectedSection === 'personal' ? 'Personal details' : sectionLabels[selectedSection as SectionType]}</Typography><Typography variant="body2" color="text.secondary">Start here, then work through each step. Changes save automatically.</Typography></Box><Box className="form-scroll">{selectedSection === 'personal' && <Alert severity="info" variant="outlined" sx={{ mb: 2 }}>Begin with your name and contact details. Then use the steps above to add the story behind your work.</Alert>}{sectionForm}</Box></Box>;
  const design = { ...defaultDesign, ...(resume.design ?? {}) };
  const updateDesign = (patch: Partial<ResumeDesign>) => updateResume({ design: { ...design, ...patch } });
  const designPanel = <Box className="editor-panel"><Box className="editor-heading"><Typography variant="h6">Design &amp; layout</Typography><Typography variant="body2" color="text.secondary">Make it yours</Typography></Box><Box className="form-scroll">
    <Typography variant="overline" color="text.secondary" className="design-section-label">Template gallery</Typography>
    <Box className="template-picker">{templates.map((item) => <Box key={item.id} className={`template-picker-option ${resume.template === item.id ? 'selected' : ''}`} onClick={() => setTemplate(item.id)} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setTemplate(item.id); }}><TemplateThumbnail template={item.id} compact /><Box className="template-picker-copy"><Typography variant="body2" fontWeight={750}>{item.label}</Typography><Typography variant="caption" color="text.secondary">{item.description}</Typography></Box>{resume.template === item.id && <Chip label="Selected" size="small" color="primary" />}</Box>)}</Box>
    ...
    <Stack direction="row" spacing={1.4} mt={1.25}>{colors.map((color) => <Box key={color} component="button" aria-label={`Use ${color} accent`} onClick={() => updateResume({ accentColor: color })} className={`accent-swatch ${resume.accentColor === color ? 'selected' : ''}`} sx={{ bgcolor: color, color }} />)}</Stack>
    ...
    <Stack spacing={2.25} mt={1.25}>
      <FormControl size="small" fullWidth><InputLabel id="font-family-label">Font family</InputLabel><Select labelId="font-family-label" label="Font family" value={design.fontFamily} onChange={(event) => updateDesign({ fontFamily: event.target.value as FontFamily })}>{fonts.map((font) => <MenuItem value={font.id} key={font.id}>{font.label}</MenuItem>)}</Select></FormControl>
      ...
      <FormControl size="small" fullWidth><InputLabel id="density-label">Content density</InputLabel><Select labelId="density-label" label="Content density" value={design.density} onChange={(event) => updateDesign({ density: event.target.value as ResumeDensity })}><MenuItem value="comfortable">Comfortable</MenuItem><MenuItem value="compact">Compact</MenuItem><MenuItem value="airy">Airy</MenuItem></Select></FormControl>
    </Stack>
    <Typography variant="overline" color="text.secondary" display="block" mt={3}>Sections</Typography><Typography variant="caption" color="text.secondary" display="block" mb={1}>Drag to reorder sections on your resume.</Typography><DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={dragEnd}><SortableContext items={resume.sections} strategy={verticalListSortingStrategy}>{resume.sections.map((section) => <SortableSection key={section} section={section} selected={false} onSelect={() => setSelectedSection(section)} />)}</SortableContext></DndContext>
  </Box></Box>;

  return <Box className="app-shell">
    <AppBar position="static" color="inherit" elevation={0} className="topbar"><Toolbar><IconButton className="topbar-icon-btn" edge="start" aria-label="Open resume navigation" onClick={() => setMobileMenu(!mobileMenu)} sx={{ display: { md: 'none' }, mr: 1 }}><MenuIcon size={20} /></IconButton><Box className="brand-mark"><Box className="brand-badge"><Sparkles size={16} fill="currentColor" /></Box><Typography component="span" fontWeight={800} letterSpacing="-0.5px" sx={{ display: { xs: 'none', sm: 'inline' } }}>ResumeForge</Typography></Box><Button size="small" color="inherit" onClick={() => navigate('/dashboard')} sx={{ textTransform: 'none', ml: 1 }}>My Resumes</Button><Box className="topbar-title"><FileText size={16} color="#64748b" /><Typography variant="body2" noWrap>{activeTitle}</Typography><Chip className={saveState === 'saving' ? 'save-chip saving' : undefined} icon={saveState === 'saving' ? <span className="save-dot" /> : undefined} label={saveState === 'saving' ? 'Saving…' : saveState === 'offline' ? 'Offline — saved locally' : 'Saved ✓'} size="small" color={saveState === 'offline' ? 'warning' : 'success'} variant="outlined" /></Box><Box flex={1} /><Tooltip title="Download a PDF copy"><Button onClick={exportPdf} startIcon={<Download size={17} />} variant="contained" size="small"><Box component="span" sx={{ display: { xs: 'none', sm: 'inline' } }}>Export PDF</Box><Box component="span" sx={{ display: { xs: 'inline', sm: 'none' } }}>Export</Box></Button></Tooltip><Tooltip title="More resume actions"><IconButton className="topbar-icon-btn" aria-label="More resume actions" onClick={(e) => setMenuAnchor(e.currentTarget)}><MoreVertical size={19} /></IconButton></Tooltip><Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={() => setMenuAnchor(null)}><MenuItem onClick={async () => { try { const result = await duplicateCloudResume(activeId); navigate(`/resume/${result.resume.id}/edit`); } catch { duplicateResume(activeId); } setMenuAnchor(null); }}>Duplicate resume</MenuItem><MenuItem onClick={() => { if (window.confirm('Delete this resume?')) { deleteCloudResume(activeId).catch(() => undefined); deleteResume(activeId); navigate('/dashboard'); } setMenuAnchor(null); }}>Delete resume</MenuItem><MenuItem onClick={() => { logout(); navigate('/login'); setMenuAnchor(null); }}>Logout</MenuItem></Menu></Toolbar></AppBar>
    <Box className="workspace">
      ...
      {(!isMobile || mobileMenu) && <Box className={`editor-side ${leftOpen ? 'with-library' : ''}`}><Tabs value={tab} onChange={(_, value) => setTab(value)} variant="fullWidth"><Tab icon={<FileText size={16} />} iconPosition="start" label="Edit content" value="editor" /><Tab icon={<Palette size={16} />} iconPosition="start" label="Design" value="design" /></Tabs>{tab === 'editor' ? <><Box className="section-nav"><Typography variant="caption" color="text.secondary" sx={{ px: 1, display: 'block', mb: .75 }}>Build your resume step by step</Typography><Box className={`section-nav-item ${selectedSection === 'personal' ? 'active' : ''}`} onClick={() => setSelectedSection('personal')}><span className="section-number">00</span>Personal details</Box><Divider />{resume.sections.map((section) => <Box key={section} className={`section-nav-item ${selectedSection === section ? 'active' : ''}`} onClick={() => setSelectedSection(section)}><span className="section-number">{sectionIcons[section]}</span>{sectionLabels[section]}</Box>)}</Box>{editorPanel}</> : designPanel}</Box>}
