...
function ResumeBuilder() {
  ...
  const designPanel = <Box className="editor-panel"><Box className="editor-heading"><Typography variant="h6">Design & layout</Typography><Typography variant="body2" color="text.secondary">Make it yours</Typography></Box><Box className="form-scroll">
    ...
    <Typography variant="overline" color="text.secondary" display="block" mt={3}>Accent color</Typography><Stack direction="row" spacing={1.2} mt={1}>{colors.map((color) => <IconButton aria-label={`Use ${color} accent`} key={color} onClick={() => updateResume({ accentColor: color })} sx={{ bgcolor: color, width: 28, height: 28, border: resume.accentColor === color ? '3px solid #dbeafe' : 'none', '&:hover': { bgcolor: color } }} />)}</Stack>
